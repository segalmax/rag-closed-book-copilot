...
free: true
scraped_at: 2026-02-23T12:15:40.967848Z
---
...
Network capabilities haven't stagnated either. Within a datacenter, 10 Gbps is standard, with high-performance instances supporting up to 20 Gbps. Cross-zone bandwidth typically ranges from 100 Mbps to 1 Gbps. Latency remains predictable: 1-2ms within a region, and 50-150ms cross-region. This consistent performance allows for reliable distributed system design.
...
Writes: 1-2ms average across data centers in the same region (e.g. AWS and Azure's "Availability Zones" or GCP's "Zones") for optimized systems

## Throughput

Reads: Hundreds of thousands of requests/second per instance for in-memory caches like ElastiCache Redis on modern Graviton-based nodes
...
Storage: Single instances handle up to 64 TiB (tebibytes) for most database engines, with Aurora supporting up to 128 TiB in some configurations
...
- 100k+ concurrent connections - 8-64 cores @ 2-4 GHz - 64-512GB RAM standard, up to 2TB

- CPU > 70% utilization - Response latency > SLA - Connections near 100k/instance - Memory > 80%

## Message Queues

- Up to 1 million msgs/sec per broker - Sub-5ms end-to-end latency - Up to 50TB storage

- Throughput near 800k msgs/sec - Partition count ~200k per cluster - Growing consumer lag
